...
//  스쿼트/푸시업/풀업 감지
// ═══════════════════════════════════════════════
function detectRep(avg) {
  const cfg = EXERCISE_CONFIG[currentEx];
  const s = smoothAngle(avg);

  if (currentEx === 'pullup') {
    if (s < cfg.upThr && phase !== 'UP') {
      if (phase === 'IDLE') repStartTime = Date.now();
      phase = 'UP'; updatePhaseUI();
    } else if (s > cfg.downThr) {
      if (phase === 'UP') finishRep();
      if (phase !== 'DOWN') { phase = 'DOWN'; updatePhaseUI(); }
    }
  } else {
    if (s < cfg.downThr && phase !== 'DOWN') {
      if (phase !== 'DOWN') repStartTime = Date.now();
...
function switchEx(ex) {
  ...
  angleHistory = []; updateCountUI(); updatePhaseUI();
